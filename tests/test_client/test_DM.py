...
def test_client():
    ...
    client.server.send('\\rAnDoMuSeRnAmE')
    time.sleep(0.5)
    assert 'User rAnDoMuSeRnAmE is not in the chat' == client.server.storage.all_messages[-1]

    dm_queue.put('ready')
    time.sleep(0.5)
    assert 'DM from Baho: Hello' == client.server.storage.all_messages[-1]
